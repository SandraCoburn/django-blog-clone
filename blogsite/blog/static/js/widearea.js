...
(function (root, factory) {
  ...
  function _clearStorage(value) {
    ...
    if (typeof value == 'string') {
      ...
      currentElement = this._targetElement.querySelector(
        'textarea[data-widearea-id="' + value + '"]'
      );
      id = value;
  ...
  function _enable() {
    var self = this,
      ...
      textAreaList = this._targetElement.querySelectorAll(
        'textarea[' + this._options.wideAreaAttr + "='enable']"
      ),
    ...
    for (var i = textAreaList.length - 1; i >= 0; i--) {
      ...
      currentTextArea.className = (
        currentTextArea.className + ' widearea'
      ).replace(/^\s+|\s+$/g, '');
      ...
      var onTextChanged = function () {
        _saveToStorage.call(
          self,
          this.getAttribute('data-widearea-id'),
          this.value
        );
      };
      ...
      } else if (currentTextArea.attachEvent) {
        //IE hack
    ...
    this._timer = setInterval(function () {
      for (var i = self._textareas.length - 1; i >= 0; i--) {
        ...
        var wideAreaIcons = document.getElementById(
          'widearea-' + currentTextArea.getAttribute('data-widearea-id')
        );
        ...
        if (
          oldPosition.left - currentTextareaPosition.width + 21 !=
            currentTextareaPosition.left ||
          oldPosition.top != currentTextareaPosition.top
        ) {
          //set icons panel position
          _renewIconsPosition(
            currentTextArea,
            wideAreaIcons,
            currentTextareaPosition
          );
        }
      }
    }, 200);
  ...
  function _renewIconsPosition(textarea, iconPanel, textAreaPosition) {
    ...
    iconPanel.style.left =
      currentTextareaPosition.left + currentTextareaPosition.width - 21 + 'px';
  ...
  function _enableFullScreen(link) {
    ...
    var targetTextarea = document.querySelector(
      "textarea[data-widearea-id='" + wideAreaId + "']"
    );
    ...
    currentTextArea.className = (
      'widearea-fullscreen ' + targetTextarea.className
    ).replace(/^\s+|\s+$/g, '');
    targetTextarea.className = (
      'widearea-fullscreened ' + targetTextarea.className
    ).replace(/^\s+|\s+$/g, '');
    ...
    overlayLayer.className =
      'widearea-overlayLayer ' + this._options.defaultColorScheme;
    ...
    this._onKeyDown = function (e) {
      ...
      if (e.keyCode == 9) {
        ...
        currentTextArea.value =
          currentTextArea.value.substring(0, selectionStart) +
          '\t' +
          currentTextArea.value.substring(currentTextArea.selectionEnd);
  ...
  function _disableFullScreen() {
    var smallTextArea = document.querySelector(
      'textarea.widearea-fullscreened'
    );
    var overlayLayer = document.querySelector('.widearea-overlayLayer');
    ...
    smallTextArea.className = smallTextArea.className
      .replace(/widearea-fullscreened/gi, '')
      .replace(/^\s+|\s+$/g, '');
